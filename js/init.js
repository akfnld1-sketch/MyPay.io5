...
function renderSettingsPage(){
  ...
  html += `<div style="background:var(--surface);border:1px solid var(--border);border-radius:12px;padding:16px 18px;margin-bottom:12px;">
    <div style="font-size:13px;font-weight:700;color:var(--text);margin-bottom:10px;">💰 급여일 설정</div>
    <div style="font-size:12px;color:var(--text3);margin-bottom:8px;line-height:1.6;">매월 급여 받는 날짜 → D-3·D-1·당일 알림</div>
    <div style="display:flex;align-items:center;gap:8px;">
      <input type="number" id="payday-input-s" min="1" max="31" placeholder="예) 25"
        value="${savedPayday}"
        style="flex:1;padding:10px 12px;border-radius:10px;border:1px solid var(--border);
               background:var(--surface2);color:var(--text);font-size:16px;font-weight:700;
               font-family:'Noto Sans KR';text-align:center;outline:none;">
      <span style="font-size:13px;color:var(--text2);">일</span>
      <button onclick="savePaydayFromSettings()"
        style="padding:10px 16px;border-radius:10px;border:none;background:var(--accent);
               color:#fff;font-size:13px;font-weight:700;cursor:pointer;font-family:'Noto Sans KR';">저장</button>
    </div>
  </div>`;
